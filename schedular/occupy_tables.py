...
def create_occupy_table(iter) -> dict[str, dict[Any, dict[int, Any]]]:
    ...
    return {
        name: {day: {int(period): None for period in value["Periods"]} for day in data.DAYS}
...
def generate_occupy_tables():
    ...
    Occupied.teachers = create_occupy_table(
        teachers.iterrows()
    )
    ...
    Occupied.grades = {
        name: {
            section: {
                day: {int(period): None for period in data.PERIODS_RANGE} for day in data.DAYS
            }
...
def occupy_course(period, course, room_name, day, course_name, section, grade): # TODO TYPES
